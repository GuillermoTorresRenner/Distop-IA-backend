---
name: Auspex
order: 1
tooltip: Sentidos sobrenaturales y percepción mística; lee auras, mentes, historia psíquica.
powers:
  - level: 1
    name: Sentidos agudizados
    summary: Amplifica un sentido a niveles sobrehumanos por una escena.
    bloodCost: 0
    rollAttribute: perception
    rollAbility: Alerta
    rollDifficulty: 6
    tooltip: "Amplifica un sentido a niveles sobrehumanos durante una escena."
  - level: 2
    name: Percepción del aura
    summary: Lee el aura emocional y sobrenatural del sujeto observado.
    bloodCost: 0
    rollAttribute: perception
    rollAbility: Empatía
    rollDifficulty: 8
    tooltip: "Lee el aura según emoción, humanidad y naturaleza sobrenatural del sujeto."
  - level: 3
    name: Tacto espiritual
    summary: Lee la historia psíquica de un objeto al tocarlo (psicometría).
    bloodCost: 0
    rollAttribute: perception
    rollAbility: Empatía
    rollDifficulty: 7
    tooltip: "Al tocar un objeto captas impresiones de su portador o eventos cargados."
  - level: 4
    name: Telepatía
    summary: Lee y proyecta pensamientos del sujeto.
    bloodCost: 1
    rollAttribute: intelligence
    rollAbility: Subterfugio
    rollDifficulty: 7
    tooltip: "Conexión mental con sujeto a la vista; lecturas profundas requieren tiradas extendidas."
  - level: 5
    name: Proyección psíquica
    summary: Separa la consciencia del cuerpo y viaja en forma astral.
    bloodCost: 1
    rollAttribute: perception
    rollAbility: Ocultismo
    rollDifficulty: 8
    tooltip: "Mente viaja en forma astral; cuerpo duerme profundo y vulnerable."
---

# Auspex

Sentidos sobrenaturales y percepción del más allá. Los Toreador la
heredan del refinamiento; los Tremere y Malkavian la usan para escarbar
en lo escondido. Cada nivel abre una puerta más profunda en la
percepción.

## 1 — Sentidos agudizados

Eliges un sentido y lo amplificas durante la escena. Penalizas a la
mitad la dificultad de cualquier tirada de Percepción con ese sentido,
pero ganas la vulnerabilidad asociada (luz cegadora, ruido ensordecedor).

## 2 — Percepción del aura

Las personas y vampiros tienen un aura coloreada según su estado
emocional, su humanidad y su naturaleza sobrenatural. Lees el color, los
remolinos, los signos de un Abrazo o de una vinculación de sangre.

## 3 — Tacto espiritual

Al tocar un objeto, capturas impresiones de su último portador o de
eventos cargados. Más éxitos = imágenes más nítidas.

## 4 — Telepatía

Estableces una conexión mental con un sujeto a la vista. Si supera tu
tirada con su Voluntad, puede bloquearte. Lecturas profundas requieren
tiradas extendidas.

## 5 — Proyección psíquica

Tu cuerpo queda en sueño profundo mientras tu mente viaja como una
forma astral semi-incorpórea. Atraviesas paredes pero no agua viva, ni
luz solar.
